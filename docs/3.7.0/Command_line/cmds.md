...
- `sel: {` [*dim*: *index*|*mask*]... `}`: Select data by an index or a mask. *dim* is a dimension name to subset along. *index* is an index or a list of indexes to select. *mask* is `true` or `false` or a list thereof, specifying which indexes to select.
